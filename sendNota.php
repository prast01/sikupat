...
if ($id_kegiatan == '6') {
	...
	$d = mysql_fetch_assoc(mysql_query("SELECT id_spj, kseksi FROM spj WHERE id_kegiatan='$id_kegiatan' AND kode_rekening='$kode_rekening' AND kseksi='$seksi'"));
}
...
if ($d['id_spj'] != '') {
	if ($rok == '0') {
		if ($id_kegiatan == '6') {
			if ($kode_rekening == '51215003' || $kode_rekening == '51218001' || $kode_rekening == '51218002') {
				...
				$q = mysql_query("INSERT INTO target_detail(id_spj, bulan, uraian, nominal, minggu, seksi, id_keg, pl, seksi_pl, st) VALUES('$d[id_spj]', '$bulan', '$alasan', '$nominal', '1', '$d[kseksi]', '$id_kegiatan', '$d2[kd_peg]', '$d[kseksi]', '2')");
			}
		} else {
