...
// $char = "TRX";
// $kodeBarang = $char . sprintf("%06s", $noUrut);
?>
	...
	<div class="panel-body" >	
		<div class="panel panel-default" style="">
			<div class="panel-heading">
				...
				<?php }elseif($act=="lkh"){ ?>
						<h3 class="panel-title">LKH PEGAWAI</h3>
			...
			<div class="panel-body">
					...
					<?php
						if(isset($_POST['simpan'])){
							...
							function simpan($kd){
								...
								if ($q['gol'] == '0') {
									...
									for ($i=0; $i < count($pegawai); $i++) {
										mysql_query("INSERT INTO bibs_detail (kd_transaksi, kd_peg, rupiah) VALUES('$kd_transaksi', '$pegawai[$i]', '$rupiah[$i]')");
									}
								}
							...
							if($hari_ini > $batas){
								...
								if ($cek == '0' && $jml == '0') {
									$hasil = mysql_query("INSERT INTO bibs(kd_transaksi, tgl_kegiatan, tgl_transfer, id_spj, kseksi, kbidang, nominal, uraian) VALUES('$kd_transaksi', '$tgl_kegiatan', '$tgl_transfer', '$id_spj', '$kseksi', '$kbidang', '$nominal', '$uraian')");
				...
				?>
					<div class="table-responsive">
						<a href="home.php?cat=data&page=registrasi-spj&act=tampil" class="btn btn-sm btn-danger">Kembali</a>
				...
				<?php
						...
						$id = $_GET['id'];
						$s = mysql_fetch_assoc(mysql_query("SELECT a.tgl_kegiatan, a.tgl_transfer, a.id_spj, b.id_kegiatan, a.nominal, a.uraian, b.gol FROM bibs a, spj b WHERE a.id_spj=b.id_spj AND a.kd_transaksi='$id'"));
				...
				<?php
						if(isset($_POST['tolak'])){
							...
							if($_POST['act'] == 'tolak'){
								...
								$hasil = mysql_query("UPDATE bibs SET tolak='0', alasan='$alasan' WHERE  kd_transaksi='$kd_transaksi'");
							}
						...
						<table id="table1" class="table table-bordered table-hover" width="100%">
							...
							<tbody>
									<?php 
										$no = 1;
										if ($kseksi == 'DJ001') {
											$query = mysql_query("SELECT a.id, a.kd_transaksi, a.tgl_kegiatan, a.tgl_transfer, a.nominal, c.kode_rekening, c.uraian_kegiatan, a.uraian, d.nm_kegiatan, a.kseksi, a.tolak, a.alasan, a.validasi FROM bibs a, spj c, kegiatan d WHERE YEAR(a.tgl_kegiatan)='$tahun' AND MONTH(a.tgl_kegiatan)='$bulan' AND a.id_spj=c.id_spj AND c.id_kegiatan=d.id_kegiatan AND a.tgl_transfer = '0000-00-00' AND a.kseksi != '' order by a.id ASC");
										} else {
											$query = mysql_query("SELECT a.id, a.kd_transaksi, a.tgl_kegiatan, a.tgl_transfer, a.nominal, c.kode_rekening, c.uraian_kegiatan, a.uraian, d.nm_kegiatan, a.kseksi, a.tolak, a.alasan, a.validasi FROM bibs a, spj c, kegiatan d WHERE YEAR(a.tgl_kegiatan)='$tahun' AND MONTH(a.tgl_kegiatan)='$bulan' AND a.id_spj=c.id_spj AND c.id_kegiatan=d.id_kegiatan AND a.kseksi='$kseksi' order by a.id ASC");
										}
												...
												<?php
														...
														if (mysql_num_rows($a) > 0) {
															...
															while ($da = mysql_fetch_assoc($a)) {
																if ($kseksi=='DJ001') {
																	echo "<li><a href='home.php?cat=data&page=registrasi-spj&act=lkh&kd_peg=$da[kd_peg]&tgl=$tgl_kegiatan'>".$da['gel_dep']." ".$da['nama']." ".$da['gel_bel']."</a></li>";
																} else {
