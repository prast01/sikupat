...
while ($hasil = mysql_fetch_array($query)) {
	...
	if ($kbidang == 'DK007') {
		...
		$a3 = mysql_query("SELECT sum(total_realisasi) as jml_total_realisasi FROM total_realisasi_fukms where kbidang2='$kbidang'") or die(mysql_error());
		$r = mysql_fetch_array($a);
		$r2 = mysql_fetch_array($a2);
		$r3 = mysql_fetch_array($a3);

		$jml_total_realisasi 	= $r['jml_total_realisasi'] + $r2['jml_total_realisasi'] + $r3['jml_total_realisasi'];
	} else {
		$a = mysql_query("SELECT sum(pagu_anggaran) as jml_pagu_anggaran FROM spj where kbidang='$kbidang' AND kbidang2=''") or die(mysql_error());
		$r = mysql_fetch_array($a);

		$jml_pagu_anggaran 	= $r['jml_pagu_anggaran'];

		$a = mysql_query("SELECT sum(total_realisasi) as jml_total_realisasi FROM spj where kbidang='$kbidang' AND kbidang2=''") or die(mysql_error());
		$a2 = mysql_query("SELECT sum(total_realisasi) as jml_total_realisasi FROM total_realisasi where kbidang='$kbidang' AND kbidang2=''") or die(mysql_error());
		$r = mysql_fetch_array($a);
		$r2 = mysql_fetch_array($a2);
						...
						<table id="table1" class="table table-bordered table-hover" width="100%">
							<thead>
								<tr>
									...
									<td style="text-align:center">
										<font color=""><b>
												<center><a href="pages/data/cetak_spj_bidang.php" target="_blank">Cetak</a></center>
											</b></font>
									</td>
								</tr>
							</thead>
