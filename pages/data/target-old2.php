...
if (isset($_GET['valid'])) {
	...
	$hasil = mysql_query("UPDATE tb_valid_rok SET $bulan='1' WHERE id_kegiatan='$id_kegiatan' AND kseksi='$kseksi'");
	$hasil2 = mysql_query("UPDATE tb_valid_rok SET $bulan='1' WHERE id_kegiatan='6' AND kseksi='$kseksi'");
...
if (isset($_GET['invalid'])) {
	...
	$hasil = mysql_query("UPDATE tb_valid_rok SET $bulan='0' WHERE id_kegiatan='$id_kegiatan' AND kseksi='$kseksi'");
	$hasil2 = mysql_query("UPDATE tb_valid_rok SET $bulan='0' WHERE id_kegiatan='6' AND kseksi='$kseksi'");

	if ($hasil) {
		echo "<script>alert('Data Berhasil Tidak Divalidasi.'); location.href='home.php?cat=data&page=target'</script>";
	}
}
		...
		<div class="panel panel-default" style="">
			...
			<form class="form-horizontal" method="post" enctype="multipart/form-data" action="">
				...
				<div class="form-group">
					...
					<div class="col-sm-4">
						<select class="form-control" name="ckkegiatan" id="keg2">
			...
			<div class="panel-body">
				<div class="panel-heading">
					<div class="row">
						...
						?>
						<div class="col-md-1">
							<a href="?cat=data&page=rekap-rok" class="btn btn-warning" target="_blank">Rekap ROK</a>
						</div>
						<?php
							if($kseksi2 == 'DJ001'){
						?>
						<div class="col-md-2">
							<a href="?cat=data&page=rekap-rok2" class="btn btn-primary" target="_blank">Rekap ROK BBM/Perdin</a>
						</div>
						<?php
							}
						?>
									...
									<tbody>
										...
										<tr>		
											...
											<td style="text-align:center">
												...
												<?php
													if($h['kseksi2'] != '' && ($id_seksi == $kseksi2 || $kseksi2 == 'DJ001')){
												?>
												<br><br>
											...
											<td>
												...
												<b>Sisa :</b> <br>
												<b><?php echo number_format($sisa, 0, ',', '.'); ?></b>											</td>
											<td>
												ROK <br><br> Realisasi											</td>
											<?php
													$nomi = 0;
														for ($i=1; $i <= 12 ; $i++) {
															$bln = sprintf("%02s",$i);
											...
											<td style="text-align:center">
												...
												?>
												<input type="number" onclick="editTarget('target','<?php echo $h['id_spj']; ?>', '<?php echo $i; ?>', '<?php echo $h['uraian_kegiatan']; ?>', 'target', '<?php echo $kseksi2; ?>', '<?php echo $id_seksi; ?>', '<?php echo $id_keg; ?>', '<?php echo $id_seksi; ?>', '<?php echo $id_keg; ?>')" readonly value="<?php echo $nominal; ?>" name="Input" id="Input" onchange="update_target(this, '<?php echo $h['id_spj']; ?>', '<?php echo $i; ?>', '<?php echo $id_keg; ?>')" style="width:110px; <?php if($i == date('m')){ echo "border:1px solid red"; } ?>" />
										  <input type="number" onclick="editTarget('realisasi','<?php echo $h['id_spj']; ?>', '<?php echo $i; ?>', '<?php echo $h['uraian_kegiatan']; ?>', 'realisasi', '<?php echo $kseksi2; ?>', '<?php echo $id_seksi; ?>', '<?php echo $id_keg; ?>')" readonly value="<?php echo $nominal2; ?>" name="" id="" style="width:110px; margin-top:5px; border:1px solid red">										  </td>
											<?php
														}
										...
										<tr>
											...
											?>
											<td style="text-align:center">
												<?php echo number_format($b[$i], 0, ',', '.'); ?>											</td>
											<?php
												}
											?>
										</tr>
										<tr>
											...
											?>
											<td style="text-align:center">
												<?php echo number_format($target[$i], 0, ',', '.'); ?>											</td>
											<?php
												}
											?>
										</tr>
										<tr>
											...
											?>
											<td style="text-align:center">
												<?php echo number_format($realisasi[$i], 0, ',', '.'); ?>											</td>
											<?php
													} else {
											?>
											<td style="text-align:center">
										...
										<tr>
											...
											?>
											<td style="text-align:center">
												<?php echo number_format($p2, 2, ',', '.'); ?>%											</td>
											<?php
													} else {
											?>
											<td style="text-align:center">
										...
										<?php
												if($_POST['ckseksi'] != 'DJ002'){
													$no = 1;
													$q = mysql_query("SELECT * FROM spj WHERE id_kegiatan='6' AND kseksi='DJ002' AND (gol='1' OR gol='2')");
													$pagu = 0;
													$sisa = 0;
													while ($h = mysql_fetch_assoc($q)) {
														$pagu = $pagu + $h['pagu_anggaran'];
										...
										<tr>		
											...
											<td style="text-align:center">
												...
												?>
												<input type="number" onclick="editTarget('target','<?php echo $h['id_spj']; ?>', '<?php echo $i; ?>', '<?php echo $h['uraian_kegiatan']; ?>', 'target', 'DJ002', 'DJ002', '6', '<?php echo $id_seksi; ?>')" readonly value="<?php echo $nominal; ?>" name="" id="<?php echo $h['id_spj'].$i; ?>" style="width:110px; <?php if($i == date('m')){ echo "border:1px solid red"; } ?>">

												<input type="number" onclick="editTarget('realisasi','<?php echo $h['id_spj']; ?>', '<?php echo $i; ?>', '<?php echo $h['uraian_kegiatan']; ?>', 'realisasi', 'DJ002', 'DJ002', '6')" readonly value="<?php echo $nominal2; ?>" name="" id="" style="width:110px; margin-top:5px; border:1px solid red">											</td>
											<?php
														}
														// $h3 = mysql_fetch_assoc(mysql_query("SELECT Sum(bibs.nominal) AS nominal FROM bibs WHERE bibs.id_spj = '$h[id_spj]' AND bibs.tgl_transfer != '0000-00-00' AND bibs.tolak = '0' AND bibs.tgl_transfer > '2019-03-31' AND MONTH(bibs.tgl_transfer) <= '8'"));
														// $h3 = mysql_fetch_assoc(mysql_query("SELECT Sum(bibs.nominal) AS nominal FROM bibs WHERE bibs.id_spj = '$h[id_spj]' AND bibs.tgl_transfer != '0000-00-00' AND bibs.tolak = '0' AND MONTH(bibs.tgl_transfer) = '9'"));
									...
									<tfoot>
										...
										<tr>
											...
											<th style="text-align:center">
												<span style="cursor:pointer"><i class="fa fa-print text-success" style="font-size:20px" title="Target ROK" onclick="cetak('1','<?php echo $i; ?>','<?php echo $_POST['ckkegiatan']; ?>','<?php echo $_POST['ckseksi']; ?>')"></i></span>
												<?php
												if($kseksi2 == 'DJ001'){
													$blnvalid = "b".$i;
												
													$q = mysql_fetch_assoc(mysql_query("SELECT $blnvalid as bulanvalid FROM tb_valid_rok WHERE id_kegiatan='$_POST[ckkegiatan]' and kseksi='$_POST[ckseksi]'"));
													$bulanvalid2 = $q['bulanvalid'];
												?>
												<br>
													<?php
													if($bulanvalid2=="1"){
													?>
													<a href="?cat=data&page=target&invalid&bulan=b<?php echo $i; ?>&kegiatan=<?php echo $_POST['ckkegiatan']; ?>&kseksi=<?php echo $_POST['ckseksi']; ?>" style="font-size:20px">
														<i class="fa fa-check-square-o" title="Validasi ROK"></i>													</a>
												<?php
													}else{
													?>
													<a href="?cat=data&page=target&valid&bulan=b<?php echo $i; ?>&kegiatan=<?php echo $_POST['ckkegiatan']; ?>&kseksi=<?php echo $_POST['ckseksi']; ?>" style="font-size:20px">
														<img width="15" src="img/no-valid.png" title="Belum Validasi" />													</a>
												<?php
													}
												}
												?>											</th>
										...
										<tr>
											...
											?>
											<th style="text-align:center">
												<span style="cursor:pointer"><i class="fa fa-print text-danger" style="font-size:20px" title="Cetak Semua ROK" onclick="cetak('2','<?php echo $i; ?>')"></i></span>											</th>
											<?php
												}
											?>
										</tr>
...
function editTarget(page, id_spj, bln, title, jenis, s1, s2, keg, id_seksi) {
	$("#modalTarget").modal();
	$('.modal-content').empty();
	var keg_pl = $('#keg2').val();
	var data = {'page':page,'id_spj': id_spj, 'bln': bln, 'title': title, 'jenis':jenis, 's1':s1, 's2':s2, 'keg':keg, 'id_seksi':id_seksi, 'keg_pl':keg_pl};
	$.ajax({
		type:'POST',
		url:'getTarget.php',
		data:data,
...
function simpanTarget(id_spj, bln) {
	...
	var keg = $('#keg').val();
	var seksi = $('#seksi').val();
	var seksi_pl = $('#seksi_pl').val();
	var pl = $('#pl').val();
	var keterangan = $('#keterangan').val();
	var keg_pl = $('#keg2').val();

	var data = {'page':page,'id_spj': id_spj, 'bln': bln, 'uraian':uraian, 'nominal':nominal, 'minggu': minggu, 'seksi': seksi, 'keg': keg, 'pl': pl, 'keterangan': keterangan, 'seksi_pl': seksi_pl, 'keg_pl': keg_pl};
	$.ajax({
		type:'POST',
		url:'getTarget.php',
		data:data,
		success:function(data){
			...
			if (json.res == 1) {
				var html = '<tr id="add-'+i+'">';
					html += '<td>'+uraian+'<br>Keterangan :<br>'+keterangan+'</td>';
					html += '<td>'+json.pl+'</td>';
					...
					html += '<td align="center"><span style="cursor:pointer" class="fa fa-pencil" title="Ubah" onclick="ubah(\'add-'+i+'\', \'add-'+i+'\', \''+uraian+'\', \''+nominal+'\', \''+minggu+'\', \''+pl+'\', \''+keterangan+'\')"></span></td>';
					html += '</tr>';
				$('#tbody').append(html);
				$('#uraian').val("");
				$('#nominal').val("");
...
function ubah(id_target, id_tr, uraian, nominal, minggu, pl, ket) {
	$("#simpan").hide();
	$("#ubah").show();
	$("#batal").show();
	$("#uraian").val(uraian);
...
function ubahTarget(id_spj, bln) {
	...
	var data = {'page':'ubahTarget','id_spj': id_spj, 'bln': bln, 'uraian':uraian, 'nominal':nominal, 'minggu': minggu, 'seksi': seksi, 'keg': keg, 'pl': pl, 'id_target': id_target, 'keterangan': keterangan};
	$.ajax({
		type:'POST',
		url:'getTarget.php',
		data:data,
...
function cetak(jenis, bln, id_keg, ckseksi) {
	if (jenis === '1') {
		window.open("cetak.php?jenis="+jenis+"&bln="+bln+"&id_keg="+id_keg+"&ckseksi="+ckseksi, "_blank");
	} else {
