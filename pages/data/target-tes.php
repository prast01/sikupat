									...
									<tbody>
										...
										<tr>		
											...
											<td style="text-align:center">
												...
												?>
												<input type="number" onclick="editTarget('target','<?php echo $h['id_spj']; ?>', '<?php echo $i; ?>', '<?php echo $h['uraian_kegiatan']; ?>', 'target', 'DJ002', 'DJ002', '<?php echo $id_keg; ?>')" readonly value="<?php echo $nominal; ?>" name="" id="<?php echo $h['id_spj'].$i; ?>" onchange="update_target(this, '<?php echo $h['id_spj']; ?>', '<?php echo $i; ?>', '<?php echo $id_keg; ?>')" style="width:110px; <?php if($i == date('m')){ echo "border:1px solid red"; } ?>">

												<input type="number" onclick="editTarget('realisasi','<?php echo $h['id_spj']; ?>', '<?php echo $i; ?>', '<?php echo $h['uraian_kegiatan']; ?>', 'realisasi', 'DJ002', 'DJ002', '<?php echo $id_keg; ?>')" readonly value="<?php echo $nominal2; ?>" name="" id="" style="width:110px; margin-top:5px; border:1px solid red">
											</td>
											<?php
														}
														// $h3 = mysql_fetch_assoc(mysql_query("SELECT Sum(bibs.nominal) AS nominal FROM bibs WHERE bibs.id_spj = '$h[id_spj]' AND bibs.tgl_transfer != '0000-00-00' AND bibs.tolak = '0' AND bibs.tgl_transfer > '2019-03-31' AND MONTH(bibs.tgl_transfer) <= '8'"));
