						...
						<table id="table1" class="table table-bordered table-hover" width="100%">
							...
							<tbody>
									<?php 
										$no = 1;
										if ($kseksi == 'DJ001') {
											$query = mysql_query("SELECT a.id, a.kd_transaksi, a.tgl_kegiatan, a.tgl_transfer, a.nominal, c.kode_rekening, c.uraian_kegiatan, a.uraian, d.nm_kegiatan, a.kseksi, a.tolak, a.alasan, a.validasi, a.tgl_terima FROM bibs a, spj c, kegiatan d WHERE YEAR(a.tgl_transfer)='$tahun' AND MONTH(a.tgl_transfer)='$bulan' AND a.id_spj=c.id_spj AND c.id_kegiatan=d.id_kegiatan AND a.tgl_transfer != '0000-00-00' AND a.kseksi != '' order by a.id ASC");
										} else {
											$query = mysql_query("SELECT a.id, a.kd_transaksi, a.tgl_kegiatan, a.tgl_transfer, a.nominal, c.kode_rekening, c.uraian_kegiatan, a.uraian, d.nm_kegiatan, a.kseksi, a.tolak, a.alasan, a.validasi, a.tgl_terima FROM bibs a, spj c, kegiatan d WHERE YEAR(a.tgl_transfer)='$tahun' AND MONTH(a.tgl_transfer)='$bulan' AND a.id_spj=c.id_spj AND c.id_kegiatan=d.id_kegiatan AND a.kseksi='$kseksi' AND a.tgl_transfer != '0000-00-00' order by a.id ASC");
										}
										$total = 0;
									...
									<tr>	
										...
										<td valign="top" align="left" bgcolor=""><?php echo $hasil['nm_kegiatan']." -<br>".$uraian; ?></td>
										...
										<a href="home.php?cat=data&page=registrasi-spj&act=setuju&id=<?php echo $kd_transaksi ?>" target="_blank"><img src='img/edit.png' title="Ubah Tanggal Transfer" /></a>
										</td>
									</tr>
								<?php }?>
							</tbody>
